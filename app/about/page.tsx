
function AboutPage() {

  return (
    <section>
      <h1 className="text-right flex gap-2 items-center justify-center text-4xl font-bold leading-none tracking-wide sm:text-6xl font-Vazir">
        ما عاشق ریاضی هستیم
      </h1>
      <p className="mt-6 text-lg tracking-wide leading-8 max-w-2xl mx-auto text-muted-foreground font-Vazir text-right">
        تیم ما با تمرکز ویژه بر دانش ریاضی و تحلیل‌های دقیق، توانسته است جایگاهی
        حرفه‌ای در زمینه‌های مختلف ریاضیاتی کسب کند. اعضای تیم با برخورداری از
        تجربه و تخصص در شاخه‌های گوناگون ریاضیات، از جمله جبر، هندسه، آمار و
        محاسبات پیشرفته، قادر به ارائه راهکارهای نوآورانه و دقیق برای مسائل
        پیچیده هستند. ما با ترکیب تفکر تحلیلی، خلاقیت و دقت علمی، پروژه‌ها و
        چالش‌های ریاضی را با بالاترین استانداردهای کیفیت حل می‌کنیم و همواره
        تلاش داریم دانش خود را در مسیر پیشرفت علمی و عملی به کار گیریم.
      </p>
    </section>
  );
}

export default AboutPage;
